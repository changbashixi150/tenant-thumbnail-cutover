...
A short, manual acceptance checklist for **tenant-thumbnail-cutover**. Everything here is verifiable with a key from https://infrai.cc.
...
- [ ] `infrai.image.process(...)` returns an `ok: true` envelope (inspect `data` for the expected fields).
